...
public class WorkerThread extends Thread {
    ...
    private Worker worker; // Add this line to store a reference to the Worker instance
    private TCPObject obj;

    public WorkerThread(Socket connection, Worker worker) { // Modify the constructor
    ...
    public void run() {
        ...
        try {
            ...
            if (obj.getTaskId() == 1){
                ...
                String filePath = "C:\\Zips\\fireBnbMain\\fireBnb\\app\\src\\main\\java\\com\\example\\firebnb\\room4322.json";

                Path path = Paths.get(filePath);

                if (Files.exists(path)) {
                    Worker.rooms = Worker.readRoomsFromJson(filePath);
                ...
                addDates();
                Worker.saveRoomsToJson(worker.connection.getLocalPort());
            }
    ...
    private synchronized void printBookings() {
        ...
        try {
            ...
            obj.setMap(reduce(obj.getListMapID(),mapping(obj.getListCategory(), Worker.rooms)));
    ...
    private synchronized void addDates() {
        ...
        if (roomToUpdate != null) {
            ...
            System.out.println("size = " + Worker.rooms.size());
    ...
    private synchronized void readRooms() throws InterruptedException {
        ...
        synchronized (Worker.rooms){
            Worker.rooms.add(obj.getRoom());
            Worker.rooms.notifyAll();
            System.out.println("notifing rooms.add(obj.getRoom());");
            System.out.println("worker.connection.getLocalPort() : "+ worker.connection.getLocalPort() + "Rooms.size = " + Worker.rooms.size());
        }

        synchronized (Worker.rooms){
            System.out.println("waiting for saveRoomsToJson(worker.connection.getLocalPort());");
            Worker.rooms.wait();
            Worker.saveRoomsToJson(worker.connection.getLocalPort()); // Call saveRoomsToJson to update the JSON file
        }
