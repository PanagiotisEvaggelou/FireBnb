...
public class WorkerThread3 extends Thread {
    ...
    private Worker3 worker; // Add this line to store a reference to the Worker instance
    private TCPObject obj;
    private List<Room> Backuprooms;

    public WorkerThread3(Socket connection, Worker3 worker) { // Modify the constructor
    ...
    public void run() {
        ...
        try {
            ...
            if(obj.getTaskId() == 8){
                Backuprooms = Worker3.rooms;
                out.writeObject(Backuprooms);
                out.flush();
            }
            else{
                ...
                if (obj.getTaskId() == 1){
                    ...
                    String filePath = "C:\\Zips\\fireBnbMain\\fireBnb\\app\\src\\main\\java\\com\\example\\firebnb\\room4324.json";

                    Path path = Paths.get(filePath);

                    if (Files.exists(path)) {
                        Worker3.rooms = Worker3.readRoomsFromJson(filePath);
                        System.out.println("JUST READ ROOMS");

                        for (Room room : Worker3.rooms){
                    ...
                    addDates();
                    Worker3.saveRoomsToJson(worker.connection.getLocalPort());
                }
    ...
    private synchronized void addGrade() {
        ...
        for (Room room : Worker3.rooms) {
            if (room.getRoomName().equals(obj.getRoomNameToGrade())) {
                roomToUpdate = room;
                Worker3.rooms.remove(room);
                break;
            }
        }
        ...
        if (roomToUpdate != null) {
            ...
            System.out.println("Room found with roomName: " + obj.getRoomNameToGrade());
            ...
            Worker3.rooms.add(roomToUpdate);
            ...
            System.out.println("Room not found with roomName: " + obj.getRoomNameToGrade());
        }
    ...
    private synchronized void printBookings() {
        ...
        try {
            requestSocket = new Socket("127.0.0.1", 4325);
            ...
            obj.setMap(reduce(obj.getListMapID(),mapping(obj.getListCategory(), Worker3.rooms)));
    ...
    private synchronized Map<List<String>, List<Room>> reduce(List<String> mapID, Map<List<String>, List<Room>> map) {
        ...
        for (Map.Entry<List<String>, List<Room>> entry : map.entrySet()) {
            if (entry.getKey().containsAll(mapID)){
                filteredMap.put(mapID, entry.getValue());
            }
        }
    ...
    private synchronized void readRooms() throws InterruptedException {
        ...
        synchronized (Worker3.rooms){
            Worker3.rooms.add(obj.getRoom());
            Worker3.rooms.notifyAll();
            System.out.println("notifing rooms.add(obj.getRoom());");
            System.out.println("worker.connection.getLocalPort() : "+ worker.connection.getLocalPort() + "Rooms.size = " + Worker3.rooms.size());
        }

        synchronized (Worker3.rooms){
            System.out.println("waiting for saveRoomsToJson(worker.connection.getLocalPort());");
            Worker3.rooms.wait();
            Worker3.saveRoomsToJson(worker.connection.getLocalPort()); // Call saveRoomsToJson to update the JSON file
        }
