...
public class WorkerThread2 extends Thread {
    ...
    private Worker2 worker; // Add this line to store a reference to the Worker instance
    private TCPObject obj;

    public WorkerThread2(Socket connection, Worker2 worker) { // Modify the constructor
    ...
    public void run() {
        ...
        try {
            ...
            if (obj.getTaskId() == 1){
                ...
                String filePath = "C:\\Zips\\fireBnbMain\\fireBnb\\app\\src\\main\\java\\com\\example\\firebnb\\room4323.json";

                Path path = Paths.get(filePath);

                if (Files.exists(path)) {
                    Worker2.rooms = Worker2.readRoomsFromJson(filePath);
                ...
                addDates();
                Worker2.saveRoomsToJson(worker.connection.getLocalPort());
            }
    ...
    private synchronized void addGrade() {
        ...
        if (roomToUpdate != null) {
            ...
            Worker2.rooms.add(roomToUpdate);
    ...
    private synchronized void printBookings() {
        ...
        try {
            ...
            obj.setMap(reduce(obj.getListMapID(),mapping(obj.getListCategory(), Worker2.rooms)));
    ...
    private synchronized void readRooms() throws InterruptedException {
        ...
        synchronized (Worker2.rooms){
            Worker2.rooms.add(obj.getRoom());
            Worker2.rooms.notifyAll();
            System.out.println("notifing rooms.add(obj.getRoom());");
            System.out.println("worker.connection.getLocalPort() : "+ worker.connection.getLocalPort() + "Rooms.size = " + Worker2.rooms.size());
        }

        synchronized (Worker2.rooms){
            System.out.println("waiting for saveRoomsToJson(worker.connection.getLocalPort());");
            Worker2.rooms.wait();
            Worker2.saveRoomsToJson(worker.connection.getLocalPort()); // Call saveRoomsToJson to update the JSON file
        }
