...
public class MasterThread extends Thread {
    ...
    private void printBookings() {
        ...
        for(int i = 1; i <= Master.WorkerNum.get(); i++) {
            ...
            port++;
            System.out.println("POOOOOO :"+port);
        ...
        try {
            requestSocket = new Socket("192.168.1.8", 4322);
            out2 = new ObjectOutputStream(requestSocket.getOutputStream());
            in2 = new ObjectInputStream(requestSocket.getInputStream());
            out2.writeObject(obj);
            out2.flush();
        ...
        try {
            requestSocket = new Socket("127.0.0.1", 4323);
            out2 = new ObjectOutputStream(requestSocket.getOutputStream());
            in2 = new ObjectInputStream(requestSocket.getInputStream());
            out2.writeObject(obj);
            out2.flush();
        ...
        try {
            requestSocket = new Socket("127.0.0.1", 4324);
            out2 = new ObjectOutputStream(requestSocket.getOutputStream());
            in2 = new ObjectInputStream(requestSocket.getInputStream());
            out2.writeObject(obj);
            out2.flush();
    ...
    private void readRooms(){
        ...
        synchronized (obj) {
            int h = hash(obj.getRoom().getRoomName());
